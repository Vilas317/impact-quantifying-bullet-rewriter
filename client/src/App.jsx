...
const App = () => {
    ...
    for (
      ...
      try {
        ...
        if (result.status === "awaiting_approval") {
           ...
           * IMPORTANT:
           *
           * The guard receives the original bullet,
           * proposed bullet, explanation AND the evidence
   ...
   *
   * sourceBullet:
   *   The original resume bullet. This must never be replaced
   *   by the evidence entered later.
   *
   * providedMetric:
  ...
  const handleRewrite = async (
    ...
    const supportingEvidence = providedMetric.trim();
    ...
    try {
      ...
      /*
       * Pass the evidence into polling so the evidence
       * guard can verify metrics generated by SuperDocs.
       */
      await pollJob(
        result.jobId,
        supportingEvidence,
  ...
  const handleMetricSubmit = async () => {
     ...
     * IMPORTANT:
     *
     * The original bullet remains `bullet`.
     *
     * The user's metric/evidence is passed separately.
     *
     * This prevents the evidence from accidentally replacing
      ...
      <main className="main">
        <section className="hero">
          <div className="eyebrow">
            <FileCheck2 size={16} />
            Evidence-preserving resume editing
          </div>
          ...
          <p>
            Rewrite resume bullets into stronger,
            outcome-focused statements without inventing
